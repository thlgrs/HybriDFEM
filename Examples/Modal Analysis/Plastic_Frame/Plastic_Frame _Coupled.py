...
plt.ylabel(r'$F / F_{max}$ or $ \omega_0/ \omega$', fontsize=18)
plt.grid()
plt.xlim([0, 100])
plt.ylim([0, 1.05])
...
for i in range(5): 
    plt.plot(U_new, w[i]/w[i,0], linewidth=0.2,linestyle='-',markerfacecolor='white',color='black',markersize=4,label=r'$\omega_' + f'{i+1}$',marker=markers[i%5])
...
plt.savefig('Omegas_vs_forcedisp.eps')
...
St.plot_modes(nb_modes, scale=20, save=True, folder=f'Coupled/Step_50', show=True)
# %%
