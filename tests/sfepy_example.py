def linear_elasticity():
    ...
    gamma_left = domain.create_region('Gamma_Left',
                                      'vertices in x < %.10f' % (min_x + eps), 'facet')
    gamma_right = domain.create_region('Gamma_Right',
                                       'vertices in x > %.10f' % (max_x - eps), 'facet')
    ...
    t_traction = Term.new('dw_surface_ltr(load.val, v)', integral, gamma_right, load=load, v=v)
...
def linear_elasticity2():
    ...
    from sfepy.discrete import Material, FieldVariable, Integral, Equation, Equations, Problem, Conditions
    from sfepy.discrete.conditions import EssentialBC
    from sfepy.solvers.ls import ScipyDirect
    from sfepy.solvers.nls import Newton
    from sfepy.terms import Term
    ...
    gamma_left  = domain.create_region('Gamma_Left',
                                       f'vertices in x < {min_x + tol:.10e}',
                                       kind='facet')
    gamma_right = domain.create_region('Gamma_Right',
                                       f'vertices in x > {max_x - tol:.10e}',
                                       kind='facet')
    ...
    t_ext = Term.new('dw_surface_ltr(load.val, v)', integral, gamma_right,
                     load=traction, v=v)
