...
class Structure_2D: 
    ...
    def make_nodes(self): 
        ...
        for i, bl in enumerate(self.list_blocks): 
            ...
            if exists: bl.make_connect(index)
            else: 
        ...
        for i, fe in enumerate(self.list_fes): 
            ...
            for j, node in enumerate(fe.nodes):
                ...
                if exists: fe.make_connect(index, j)
                else: 
    ...
    def make_cfs(self, lin_geom, nb_cps=2, offset=-1, contact=None, surface=None):
        ...
        for contactface in interfaces: 
            ...
            self.list_cfs.append(cf.CF_2D(contactface,nb_cps,lin_geom,offset=offset,contact=contact,surface=surface))
    ...
    def fixNode(self, node_ids, dofs): 
        ...
        if isinstance(node_ids, int): # Loading one single node
            ...
            if node_fixed < 0: warnings.warn('Input node to be fixed does not exist')
            else: 
                if isinstance(dofs, int): 
        ...
        else: warnings.warn('Nodes to be loaded must be int, list of ints or numpy array')
    ...
    def loadNode(self, node_ids, dofs, force, fixed=False): 
        ...
        def load_onenode(Str, node_id, dof, force, fixed=False): 
            ...
            index = 3*node_id + dof
            if fixed: Str.P_fixed[index] = force
            else: Str.P[index] = force
        ...
        if isinstance(node_ids, int): 
            ...
            if node_loaded < 0: warnings.warn('Input node to be loaded does not exist')
    ...
    def solve_dispcontrol(self, steps, disp, node, dof, tol=1, stiff='tan', max_iter=100, filename='Results_ForceControl', dir_name=''):
            ...
            while not converged: 
                ...
                if res < tol: 
                    ...
                    try: 
                        ...
                        dU_ref[self.dof_free] = np.linalg.solve(self.K[np.ix_(self.dof_free, self.dof_free)], self.P[self.dof_free])
    ...
    def get_C_str(self):
        ...
        if self.damp_type == 'RAYLEIGH':
            ...
            try : self.solve_modal(modes=2, save=False)
            except : self.solve_modal(save=False)
            ...
            try : self.solve_modal(modes=1, save=False)
            except : self.solve_modal(save=False)
            self.get_K_str()
            # print(self.xsi)
    ...
    def ask_method(self, Meth=None):
        ...
        if Meth is None: 
            ...
            if Meth == 'CDM' or Meth == '': return Meth, None 
            elif Meth == 'CAA' or Meth == 'NWK': return 'NWK', {'g': 1/2, 'b': 1/4}  # If not specified run CAA by default
            elif Meth == 'LA': return 'NWK', {'g': 1/2, 'b': 1/6}
            elif Meth == 'NWK': 
                ...
                if t < 1: warnings.warn('Theta should be larger or equal to one for Wilson\'s theta method')
                elif t < 1.37: warnings.warn('Theta should be larger or equal to one for unconditional stability in Wilson\'s theta method')
                return 'WIL',  {'t': t}
                ...
                if a < 0 or a > 1/3: warnings.warn('Alpha should be between 0 and 1/3 for unconditional stability in HHT Method')
                if g=='': g = (1+2*a)/2
                else: g = float(g)
                if b=='': b = (1+a)**2/4
                else: b = float(b)
                ...
                if a > 1/2: warnings.warn('Alpha should be smaller thann 1/2 for unconditional stability in WBZ Method')
                if g=='': g = (1-2*a)/2
                else: g = float(g)
                if g < (1-2*a)/2: warnings.warn('Gamma should be larger than (1-2a)/2 for unconditional stability in WBZ Method')
                if b=='': b = 1/4
                else: b = float(b)
                ...
                return 'GEN', {'am': (2*m-1)/(m+1), 'af': m/(m+1), 'g': (3*m-1)/(2*(m+1)), 'b': (m/(m+1))**2}
           
        elif isinstance(Meth, str): 
            
            if Meth == 'CDM': return Meth, {} 
            elif Meth == 'CAA' or Meth == 'NWK': return 'NWK', {'g': 1/2, 'b': 1/4}  # If not specified run CAA by default
            elif Meth == 'LA': return 'GEN', {'am': 0, 'af': 0, 'g': 1/2, 'b': 1/6}
            elif Meth == 'NWK': return 'GEN', {'am': 0, 'af': 0, 'g': 1/2, 'b': 1/4}
            elif Meth == 'WIL': return 'WIL',  {'t': 1.5}
            elif Meth == 'HHT': return 'GEN', {'am': 0, 'af': 0, 'g': 1/2, 'b': 1/4}
            elif Meth == 'WBZ': return 'GEN', {'am': 0, 'af': 0, 'g': 1/2, 'b': 1/4}
            elif Meth == 'GEN': 
                m = 1
                return 'GEN', {'am': (2*m-1)/(m+1), 'af': m/(m+1), 'g': (3*m-1)/(2*(m+1)), 'b': (m/(m+1))**2}
            ...
            if Meth[0] == 'NWK': 
                ...
                else: warnings.warn('Requiring 3 parameters for HHT method')
    ...
    def solve_dyn_linear(self, T, dt, U0=None, V0=None, lmbda=None, Meth=None, filename='', dir_name=''):
        ...
        if Meth == 'CDM': 
            ...
            for i in np.arange(1,nb_steps): 
                ...
                A_conv[self.dof_free,i] = (U_conv[self.dof_free,i] - 2*U_conv[self.dof_free,i-1] + U_conv[self.dof_free,i-2]) / (dt**2)
                P_conv[:,i] = P_h.copy()
            ...
            for i in np.arange(1, nb_steps): 
                ...
                U_conv[self.dof_free,i] = np.linalg.solve(K_h, P_h)[self.dof_free]
                V_conv[self.dof_free,i] = (P['g']/(P['b']*dt)) * (U_conv[self.dof_free,i] - U_conv[self.dof_free,i-1]) + (1-P['g']/P['b']) * V_conv[self.dof_free,i-1] + dt*(1-P['g']/(2*P['b'])) * A_conv[self.dof_free,i-1]
                A_conv[self.dof_free,i] = (1/(P['b']*dt**2)) * (U_conv[self.dof_free,i] - U_conv[self.dof_free,i-1]) - V_conv[self.dof_free,i-1] / (P['b']*dt) - (1/(2*P['b'])-1) * A_conv[self.dof_free,i-1]
                P_conv[:,i] = self.K0 
            ...
            for i in np.arange(1,nb_steps):
                ...
                d_A = (6/(P['t']*dt)**2 * d_Uh - 6/(P['t']*dt) * V_conv[:,i-1] - 3 * A_conv[:,i-1])/(P['t'])
            ...
            A3 = (1-P['am']-2*P['b'])/(2*P['b']) * self.M + dt*(1 + P['af'])*(P['g']-2*P['b'])/(2*P['b']) * self.C
            
            K_h = self.K0 * (1 + P['af']) + A1
            
            for i in np.arange(1,nb_steps):
                
                P_h = loading[i]*self.P + A1 @ U_conv[:,i-1] + A2 @ V_conv[:,i-1] + A3 @ A_conv[:,i-1] + P['af'] * self.K0 @ U_conv[:,i-1]
                   
                U_conv[:,i][self.dof_free] = (np.linalg.solve(K_h,P_h))[self.dof_free]
                V_conv[:,i][self.dof_free] = (P['g']/(P['b']*dt)*(U_conv[:,i] - U_conv[:,i-1]) + (1-P['g']/P['b'])*V_conv[:,i-1] + dt * (1-P['g']/(2*P['b'])) * A_conv[:,i-1])[self.dof_free]   
                A_conv[:,i][self.dof_free] = (1/(P['b']*dt**2)*(U_conv[:,i] - U_conv[:,i-1]) - 1/(dt*P['b'])*V_conv[:,i-1] - (1/(2*P['b'])-1) * A_conv[:,i-1] )[self.dof_free]   
                P_conv[:,i] = P_h.copy()
    ...
    def solve_dyn_nonlinear(self, T, dt, U0=None, V0=None, lmbda=None, Meth=None, filename='', dir_name=''):
        ...
        if Meth == 'CDM': 
            ...
            A_conv[:,0] = np.linalg.solve(self.M, loading[0]*self.P + self.P_fixed - self.C @ V_conv[:,0] - F_conv[:,0])
            ...
            for i in np.arange(1,nb_steps):
                ...
                P_h = loading[i-1]*self.P + self.P_fixed - A @ U_conv[:,i-2] + B @ U_conv[:,i-1] - F_conv[:,i-1]
                
                U_conv[self.dof_free,i] = np.linalg.solve(K_h, P_h)[self.dof_free] 
               
                V_conv[self.dof_free,i] = ((U_conv[:,i] - U_conv[:,i-2])/(2*dt))[self.dof_free]
                A_conv[self.dof_free,i] = ((U_conv[:,i] - 2*U_conv[:,i-1] + U_conv[:,i-2])/(dt**2))[self.dof_free]
            ...
            A3 = ((1-P['am'])/(2*P['b'])-1) * self.M + dt * (1+P['af']) * (P['g']/(P['b']*dt)-1)*self.C
            ...
            for i in np.arange(1, nb_steps): 
                ...
                P_h = loading[i]*self.P + self.P_fixed + A1 @ self.U + A2 @ V_conv[:,i-1] + A3 @ A_conv[:,i-1] + P['af']*F_conv[:,i]
                ...
                V_conv[self.dof_free,i] = ((P['g']/(P['b']*dt))*(U_conv[:,i]-U_conv[:,i-1]) + (1-P['g']/P['b'])*V_conv[:,i-1] + dt*(1-P['g']/(2*P['b']))*A_conv[:,i-1])[self.dof_free]
                A_conv[self.dof_free,i] = (1/(P['b']*dt**2)*(U_conv[:,i]-U_conv[:,i-1]) - (1/(dt*P['b']))*V_conv[:,i-1]- (1/(2*P['b'])-1)*A_conv[:,i-1])[self.dof_free]
    ...
    def solve_modal(self, modes=None, no_inertia=False, filename='Results_Modal', dir_name='', save=True):
        ...
        if modes is None: 
            omega, phi = sc.linalg.eigh(self.K[np.ix_(self.dof_free, self.dof_free)], self.M[np.ix_(self.dof_free, self.dof_free)])  
        elif isinstance(modes, int): 
            if np.linalg.det(self.M) == 0: warnings.warn('Might need to use linalg.eig if the matrix M is non-invertible')
            omega, phi = sc.sparse.linalg.eigsh(self.K[np.ix_(self.dof_free, self.dof_free)], modes, self.M[np.ix_(self.dof_free, self.dof_free)],which='SM')
    ...
    def detect_interfaces(self): 
        ...
        def detect_interface_2blocks(cand, anta): 
            ...
            if len(interfaces) == 0: return False, None
            else: return True, interfaces
        
        def distance(a, b): return np.sqrt((a[0]-b[0])**2 + (a[1]-b[1])**2)
        ...
        for i, cand in enumerate(self.list_blocks): 
            ...
            for j, anta in enumerate(self.list_blocks[i+1:]):
                ...
                if distance(cand.circle_center, anta.circle_center) >= (cand.circle_radius + anta.circle_radius)*1.01: continue
    ...
    def plot_stiffness(self, save=None): 
        ...
        def normalize(smax, smin): 
            ...
            if (smax-smin) == 0 and smax < 0: return Normalize(vmin=1.1*smin/1e9, vmax=0.9*smax/1e9, clip=False)
            elif (smax-smin) == 0 and smax == 0: return Normalize(vmin=-1e-6, vmax=1e-6, clip=False)
            elif (smax-smin) == 0: return Normalize(vmin=0.9*smin/1e9, vmax=1.1*smax/1e9, clip=False)
            else: return Normalize(vmin=smin/1e9, vmax=smax/1e9, clip=False) 
        ...
        def plot(stiff, vertex): 
            ...
            for i in range(len(stiff)): 
                if smax-smin == 0: index=norm(np.around(stiff[i],6)/1e9)
                else: index = norm(np.around(stiff[i],6)/1e9)
                color = cmap(index)
    ...
    def plot_stresses(self, angle=None, save=None): 
        ...
        def normalize(smax, smin): 
            ...
            if (smax-smin) == 0 and smax < 0: return Normalize(vmin=1.1*smin/1e6, vmax=0.9*smax/1e6, clip=False)
            elif (smax-smin) == 0 and smax == 0: return Normalize(vmin=-1e-6, vmax=1e-6, clip=False)
            elif (smax-smin) == 0: return Normalize(vmin=0.9*smin/1e6, vmax=1.1*smax/1e6, clip=False)
            else: return Normalize(vmin=smin/1e6, vmax=smax/1e6, clip=False) 
        ...
        def plot(stress, vertex, name_stress=None): 
            ...
            for i in range(len(sigma)): 
                if smax-smin == 0: index=norm(np.around(stress[i],6)/1e6)
                else: index = norm(np.around(stress[i],6)/1e6)
                color = cmap(index)
                vertices_x = np.append(vertex[i][:,0], vertex[i][0,0])
                vertices_y = np.append(vertex[i][:,1], vertex[i][0,1])
                plt.fill(vertices_x, vertices_y, color=color)
    ...
    def plot_structure(self, scale=0, plot_cf=True, plot_forces=True, plot_supp=True, show=True, save=None, lims=None):
        ...
        if lims is None: 
            ...
            else: plt.figure(None, dpi=400, figsize=(6*x_len/y_len,6))
        ...
        if plot_forces: 
            for i in self.dof_free: 
                ...
                if self.P[i] != 0: 
                    ...
                    start = self.list_nodes[node_id] + scale*self.U[3*node_id*np.ones(2,dtype=int) + np.array([0,1],dtype=int)]
                    arr_len = .3
                    
                    if dof == 0: 
                        end = arr_len * np.array([1,0]) * np.sign(self.P[i])
                        plt.arrow(start[0], start[1], end[0], end[1], head_width=0.05, head_length=0.075, fc='green', ec='green')
                    elif dof == 1: 
                        end = arr_len * np.array([0,1]) * np.sign(self.P[i])
                        plt.arrow(start[0], start[1], end[0], end[1], head_width=0.05, head_length=0.075, fc='green', ec='green')
                    else:  
                        if np.sign(self.P[i]) == 1: 
                            plt.plot(start[0], start[1], marker='o', markerfacecolor='None', markeredgecolor='green', markersize=10)
                            plt.plot(start[0], start[1], marker='.', markerfacecolor='green', markeredgecolor='green', markersize=5)
                        else: 
                            plt.plot(start[0], start[1], marker='o', markerfacecolor='None', markeredgecolor='green', markersize=10)
                            plt.plot(start[0], start[1], marker='x', markerfacecolor='None', markeredgecolor='green', markersize=10)
                ...
                if self.P_fixed[i] != 0: 
                    ...
                    if dof == 0: 
                        ...
                        plt.arrow(start[0], start[1], end[0], end[1], head_width=0.05, head_length=0.075, fc='red', ec='red')
                    elif dof == 1: 
                        end = arr_len * np.array([0,1]) * np.sign(self.P_fixed[i])
                        plt.arrow(start[0], start[1], end[0], end[1], head_width=0.05, head_length=0.075, fc='red', ec='red')
                    else:  
                        if np.sign(self.P_fixed[i]) == 1: 
                            plt.plot(start[0], start[1], marker='o', markerfacecolor='None', markeredgecolor='red', markersize=10)
                            plt.plot(start[0], start[1], marker='.', markerfacecolor='red', markeredgecolor='red', markersize=5)
                        else: 
                            plt.plot(start[0], start[1], marker='o', markerfacecolor='None', markeredgecolor='red', markersize=10)
                            plt.plot(start[0], start[1], marker='x', markerfacecolor='None', markeredgecolor='red', markersize=10)
        ...
        if plot_supp: 
            ...
            for fix in self.dof_fix: 
                ...
                node = self.list_nodes[node_id] + scale*self.U[3*node_id*np.ones(2,dtype=int) + np.array([0,1],dtype=int)]
                
                import matplotlib as mpl
                
                if dof == 0: mark = mpl.markers.MarkerStyle(marker=5)
                elif dof == 1: mark = mpl.markers.MarkerStyle(marker=6)
                else: mark = mpl.markers.MarkerStyle(marker="x")
